...
def plot_correlation_heatmap(
    ...
    *,
    ...
    fig, ax = plt.subplots(figsize=size)
    # ``imshow`` defaults to ``aspect='equal'``, which forces a square matrix.
    # Notebook renderers then crop the unused canvas width, making different
    # configured figure sizes appear identical. ``auto`` lets the heatmap use
    # the full configured width and height.
    im = ax.imshow(masked, cmap="RdYlGn_r", vmin=-1, vmax=1, aspect="auto")
